const POST = {
    DEFAULT: '/',
    CREATE_ETH2SOL_MIX: '/create-eth2sol-mix',
    CREATE_SOL2ETH_MIX: '/create-sol2eth-mix',
    DEPOSIT_ETH: '/deposit-eth',
    VALIDATE_ETH_DEPOSIT: '/validate-eth-deposit',
    VALIDATE_SOL_DEPOSIT: '/validate-sol-deposit',
    DEPOSIT_SOL: '/deposit-sol',
    DEPOSIT_ETH_TOKEN: '/deposit-eth-token',
    DEPOSIT_SOL_TOKEN: '/deposit-sol-token',
    WITHDRAW_ETH: '/withdraw-eth',
    WITHDRAW_SOL: '/withdraw-sol',
    WITHDRAW_ETH_TOKEN: '/withdraw-eth-token',
    WITHDRAW_SOL_TOKEN: '/withdraw-sol-token',
    SET_SOL_MAINTAINER: '/set-sol-maintainer',
    SET_SOL_FEECOLLECTOR: '/set-sol-feecollector',
    SET_SOL_REFUND: '/set-sol-refund',
    SET_SOL_FEE: '/set-sol-fee',
    SET_SOL_MINDEPOSIT: '/set-sol-mindeposit',
    SET_SOL_MINTOKENDEPOSIT: '/set-sol-mintokendeposit',
    SEND_TRANSACTION: '/send-transaction',
    GET_LATEST_BLOCK: '/get-latest-block'
}

export default POST