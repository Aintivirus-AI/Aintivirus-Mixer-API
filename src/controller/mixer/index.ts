...
class MixerController {
  static depositETH = async (
    ...
    try {
      ...
      const isNative =
        currency === MIX_CONFIG.ADDRESS.ETH_COIN_ADDRESS ? true : false;
      ...
      const aintiVirusMixer = new AintiVirusMixer(
      ...
      const zkPreProofData = await ZkSnark.createPreProof(
        secret,
        nullifier,
        currency,
        amountInWei
      ...
      if (isNative) {
        const depositTransaction =
          await aintiVirusMixer.populateTransactionDeposit(
            currency,
            ...
            CryptoUtil.bigIntToBytes32(commitment),
            {
              ...
              pubSignals: zkPreProofData.publicSignals.map((signal) =>
                signal.toString()
        ...
        const approveTransaction =
          await erc20Standard.populateTransactionApprove(
            MIX_CONFIG.ADDRESS.MIXER_CONTRACT_ADDRESS,
            amountInWei.toString()
          );
        const depositTransaction =
  ...
  static depositSOL = async (
    ...
    try {
      ...
      const isNative =
        currency === MIX_CONFIG.ADDRESS.SOL_COIN_ADDRESS ? true : false;
      const operatorSOLWallet = Keypair.fromSecretKey(
        base58.decode(ENV.SOL_POOL_PRIVKEY)
      );
      ...
      if (isNative) {
        ...
        transaction = await solanaSDK.buildSendSOLTransaction(
          operatorSOLWallet.publicKey.toString(),
          amount,
          sender
        );

        const ethPrice = await CoinMarketcapAPI.getQuoteBySymbol(
          "SOL",
          "ETH",
          amount
        );
        ...
        transaction = await solanaSDK.buildSendSPLTransaction(
          currency,
          operatorSOLWallet.publicKey.toString(),
          Number(amount),
          sender
  ...
  static validateETHDeposit = async (
    payload: RequestPayload
  ): Promise<ResponsePayload> => {
    try {
      ...
      if (tx.from.toLowerCase() !== session.sender.toLowerCase()) {
        throw Boom.internal(
          `Error: Invalid transaction sender. Expected ${session.sender} but got ${tx.from}`
        );
      }
      if (
        tx.to.toLowerCase() !==
        MIX_CONFIG.ADDRESS.MIXER_CONTRACT_ADDRESS.toLowerCase()
      ) {
        throw Boom.internal(
          `Error: Invalid transaction recipient. Expected ${MIX_CONFIG.ADDRESS.MIXER_CONTRACT_ADDRESS} but got ${tx.to}`
        );
      }
      if (parsedTx.name !== "deposit") {
        throw Boom.internal(
          `Error: Invalid transaction function. Expected deposit but got ${parsedTx.name}`
        );
      }
      if (
        parsedTx.args[0].toString().toLowerCase() !==
        session.currency.toLowerCase()
      ) {
        throw Boom.internal(
          `Error: Invalid transaction argument(currency). Expected ${
            session.currency
          } but got ${parsedTx.args[0].toString()}`
        );
      }
      if (
        BigInt(parsedTx.args[1]).toString() !==
        BigInt(session.amount).toString()
      ) {
        throw Boom.internal(
          `Error: Invalid transaction argument(amount). Expected ${BigInt(
            session.amount
          ).toString()} but got ${BigInt(parsedTx.args[1]).toString()}`
        );
      }
  ...
  static validateSOLDeposit = async (
    payload: RequestPayload
  ): Promise<ResponsePayload> => {
    try {
      const { sessionId, txHash } = payload;
      ...
      for (const ix of compiledInstructions) {
        ...
        if (!txInfo || !txInfo.meta)
          throw Boom.internal("Transaction not found");
        ...
        if (programId.equals(SystemProgram.programId)) {
          ...
          if (actualTransferAmount !== Number(session.amount)) {
            throw Boom.internal(
              `Error: Invalid transaction argument(amount). Expected: ${session.amount}, Actual: ${actualTransferAmount}`
            );
          }
          ...
          if (
            to.toBase58().toLowerCase() !==
            operatorSOLWallet.publicKey.toString().toLowerCase()
          ) {
            throw Boom.internal(
              `Error: Invalid transaction receiver. Expected: ${operatorSOLWallet.publicKey.toString()}, Actual: ${to.toBase58()}`
            );
          }
        }
        ...
        if (programId.equals(TOKEN_PROGRAM_ID)) {
          ...
          if (
            ...
            throw Boom.internal(
              `Error: Invalid transaction argument(currency). Expected: ${
                session.currency
              }, Actual: ${mintAddress.toBase58()}`
            );
          }
          if (
            toTokenAccountInfo.owner.toBase58().toLowerCase() !==
            operatorSOLWallet.publicKey.toString().toLowerCase()
          ) {
            throw Boom.internal(
              `Error: Invalid transaction receiver. Expected: ${operatorSOLWallet.publicKey.toString()}, Actual: ${toTokenAccountInfo.owner.toBase58()}`
            );
          }
        }
      }
      ...
      const tx = await aintiVirusMixer.addCommitmentForEthWithdrawal(
        CryptoUtil.bigIntToBytes32(BigInt(session.commitment))
      );
      await tx.wait();
  ...
  static withdrawSOL = async (
    payload: RequestPayload
  ): Promise<ResponsePayload> => {
    try {
      const { note, receiver } = payload;
      const noteObj: NoteObj = JSON.parse(
        Buffer.from(base58.decode(note)).toString("utf8")
      );

      const preproofValidation = await ZkSnark.offchainVerifyPreProof(
        noteObj.zkData
      );
      ...
      const merkleReConstructor = new MerkleTreeReconstructor(
        MIX_CONFIG.ADDRESS.MIXER_CONTRACT_ADDRESS,
        ENV.ETHEREUM_RPC_URL
      );

      const nullifierHash = ZkSnark.computeNullifierHash(
      ...
      const leafIndex = await aintiVirusMixer.getSolLeafIndexByCommitment(
        CryptoUtil.bigIntToBytes32(commitment)
      );
      ...
      const { root, path } = await merkleReConstructor.getLastRootAndMerklePath(
        TreeType.SOL,
        leafIndex
      );
      ...
      const merkleVerification = await merkleVerifier.verifyMerkleProof(
        CryptoUtil.bigIntToBytes32(commitment),
      ...
      const validationTx = await aintiVirusMixer.verifySolWithdrawal(root, {
        pA: withdrawalProof.calldata.a,
        pB: withdrawalProof.calldata.b,
        pC: withdrawalProof.calldata.c,
        pubSignals: withdrawalProof.publicSignals.map((signal) =>
          signal.toString()
        ) as [
          string,
          string,
      ...
      if (currency === MIX_CONFIG.ADDRESS.ETH_COIN_ADDRESS) {
        ...
        const solAmount = await CoinMarketcapAPI.getQuoteBySymbol(
          "ETH",
          "sol",
          formattedAmount
        );
        try {
          ...
          const tx = await aintiVirusMixer.setNullifierForSolWithdrawal(
        ...
        const formattedAmount = ethers.formatUnits(
          noteObj.zkData.publicSignals[4],
          decimals
        );

        try {
          const txSig = await solanaSDK.sendSPLToken(
            MIX_CONFIG.ETH2SOL_CURRENCY_MAP[currency],
            receiver,
  ...
  static withdrawETH = async (
    ...
    try {
      ...
      const leafIndex = await aintiVirusMixer.getEthLeafIndexByCommitment(
        CryptoUtil.bigIntToBytes32(commitment)
      );
      const { root, path } = await merkleReConstructor.getLastRootAndMerklePath(
        TreeType.ETH,
